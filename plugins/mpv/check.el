...
(defmacro rmpv--setf (&rest args)
  "Set multiple session fields atomically."
  (declare (indent 1))
  (when rmpv-session
    `(setf ,@args)))
...
(defun rmpv--handle-message (msg)
  (rmpv--with-session
   (let ((event (alist-get 'event msg))
         (reply-id (alist-get 'reply_id msg))
     ...
     (when (and (listp data) (string= (alist-get 'type data) "time"))
       (setf (rmpv-session-last-time rmpv-session)
...
(defun rmpv--connect ()
  (rmpv--with-session
   (let ((socket-path (rmpv-session-socket-path rmpv-session)))
     (when (and socket-path (file-exists-p socket-path))
       (condition-case-unless-debug err
         ...
         (error
          (rmpv--debug "connection failed: %s, retrying..." err)
          (run-with-timer 0.2 nil #'rmpv--connect)))))))

(defun rmpv--observe-time-pos ()
  "Start observing time-pos property."
  (rmpv--with-session
   (let ((req-id (1+ (rmpv-session-command-id rmpv-session))))
     (setf (rmpv-session-command-id rmpv-session) req-id)
     (rmpv--send `((command . ["observe_property" 1 "time-pos"])
                   (request_id . ,req-id))))))
...
;;;###autoload
  (defun rmpv-pause ()
    "Pause playback."
    (interactive)
    (rmpv--send '((command . ["set" "pause" "yes"]))))

;;;###autoload
  (defun rmpv-resume ()
    "Resume playback."
    (interactive)
    (rmpv--send '((command . ["set" "pause" "no"]))))
...
;;;###autoload
  ...
  (defun rmpv-get-time (&optional callback)
    ...
    (rmpv--with-session
     (let ((req-id (1+ (rmpv-session-command-id rmpv-session))))
       (setf (rmpv-session-command-id rmpv-session) req-id)
       (rmpv--send `((command . ["get_property" "time-pos"]) (request_id . ,req-id)))
       (if callback
...
;;;###autoload
  (defun rmpv-seek-forward (&optional seconds)
    ...
    (interactive "p")
    (rmpv--send `((command . ["seek" ,(or seconds 10) "relative"]))))
...
;;;###autoload
  (defun rmpv-load-subtitle (path)
    ...
    (rmpv--send `((command . ["sub-add" ,(expand-file-name path)]))))
...
;;;; Subtitle Functions
  ...
  (defun rmpv-parse-timestamp (ts)
    ...
    (when (and ts (stringp ts) (not (string-blank-p ts)))
      (let ((cleaned (replace-regexp-in-string "[ \t\r\n]+" "" ts)))
	(when (string-match (concat "\\`\\([0-9]+\\):\\([0-9]+\\):"
                                    "\\([0-9]+\\)\\([,.]\\)\\([0-9]+\\)\\'") cleaned)
  ...
  (defun rmpv-parse-srt-from-string (content)
    ...
    (seq-into
     (seq-map (lambda (block)
		(when-let* ((start-str (and (string-match (concat "\\`[0-9]+[ \t]*\n"
                                                                  "\\([0-9]+:[0-9]+:[0-9]+[,.:][0-9]+\\)"
                                                                  "[ \t]+-->?[ \t]+"
  ...
  (defun rmpv-find-subtitle-at-time (time)
    ...
    (when-let ((entries (and rmpv-session (rmpv-session-subtitle-entries rmpv-session)))
               ((numberp time)))
      (seq-find (lambda (entry)
                  (let ((start (rmpv--subtitle-start entry))
			(end (rmpv--subtitle-end entry)))
  ...
  (defun rmpv-get-subtitle-index (time)
    ...
    (when rmpv-session
      (let ((entry (rmpv-find-subtitle-at-time time)))
	(when entry
  ...
  (defun rmpv-update-subtitle-highlight ()
    ...
    (rmpv--with-session
     (let* ((time (rmpv-session-last-time rmpv-session))
            ...
            (frame-valid (and frame (frame-live-p frame))))
       (when (and (numberp time) (>= time 0) entries frame-valid)
  ...
  (defun rmpv-subtitle-stop-update ()
    ...
    (when rmpv-session
      (when (rmpv-session-subtitle-timer rmpv-session)
	(cancel-timer (rmpv-session-subtitle-timer rmpv-session))
	(setf (rmpv-session-subtitle-timer rmpv-session) nil))
      (when (rmpv-session-subtitle-frame rmpv-session)
	(when (frame-live-p (rmpv-session-subtitle-frame rmpv-session))
          (delete-frame (rmpv-session-subtitle-frame rmpv-session)))
