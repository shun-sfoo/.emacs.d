#+TITLE: Emacs Lisp 现代最佳实践指南
#+AUTHOR: Emacs Lisp 开发者
#+DATE: 2024
#+OPTIONS: toc:t num:t
...
#+BEGIN_QUOTE
本文档详细介绍了 Emacs Lisp 编程的现代最佳实践，包括新旧写法对比、API 用法说明等。
#+END_QUOTE

* 目录

1. [[#1-词法作用域-lexical-binding][词法作用域 (lexical-binding)]]
2. [[#2-seq-库][seq 库]]
3. [[#3-defcustom-与-defgroup][defcustom 与 defgroup]]
4. [[#4-cl-defstruct-结构体][cl-defstruct 结构体]]
5. [[#5-autoload-延迟加载][autoload 延迟加载]]
6. [[#6-pcase-模式匹配][pcase 模式匹配]]
7. [[#7-define-minor-mode][define-minor-mode]]
8. [[#8-condition-case-错误处理][condition-case 错误处理]]
9. [[#9-命名空间前缀][命名空间前缀]]
10. [[#10-标准包头部][标准包头部]]
