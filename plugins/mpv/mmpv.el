...
(defconst mmpv--connect-retry-delay 0.2
  "Delay in seconds before retrying socket connection.")

(defconst mmpv--connect-wait-delay 0.1
  "Delay in seconds before attempting socket connection.")

(defconst mmpv--sync-timeout 0.5
  "Timeout in seconds for synchronous property reads.")
...
(defun mmpv--connect ()
  (when (and mmpv-session
    ...
    (condition-case-unless-debug _
      ...
      (error
       (run-with-timer mmpv--connect-retry-delay nil #'mmpv--connect)))))
              ...
              (make-process
               ...
               :sentinel (lambda (_p s)
                            ...
                            (when (string-match "exited\\|terminated\\|closed" s)
                              (mmpv--cleanup)))))
        (run-with-timer mmpv--connect-wait-delay nil #'mmpv--connect)
        (mmpv-mode 1))
    (unless mmpv-session
...
(defun mmpv-get-time (&optional callback)
  ...
  (when mmpv-session
    (if callback
      ...
      (let ((end (+ (float-time) mmpv--sync-timeout)))
